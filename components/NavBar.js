...
const NavBar = () => {
  ...
  return (
    ...
    <div className='flex justify-between items-center m-6 md:m-0 md:hidden' onClick={()=>setOpen(!open)}>
        ...
        <div className=" flex-row justify-between  max-w-3xl md:my-12 md:mx-auto mb-8 w-full text-gray-700 md:flex">
            <div className={`flex flex-col md:justify-start md:items-center md:flex-row pt-6 md:pt-0 text-lg space-y-2 md:space-y-0 md:space-x-8  absolute mx-3 w-[94%] md:w-full md:pl-0 pl-9 transition-all duration-500 md:static bg-gray-200 md:bg-white rounded-2xl md:z-auto z-[1000] pb-8 md:pb-0 ${open ? 'top-20' : 'top-[-490px]'}`}>
